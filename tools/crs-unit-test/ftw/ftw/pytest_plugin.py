...
def get_testdata(rulesets, test_title=None):
    """
    ...
    for ruleset in rulesets:
        for test in ruleset.tests:
            if test_title and test.test_title != test_title:
                continue
            if test.enabled:
...
def test_title(request):
    ...
    """
    return request.config.getoption('--test_title')
...
def pytest_addoption(parser):
    ...
    parser.addoption('--protocol', action='store', default=None,
                     ...
                     choices=['http', 'https'])
    parser.addoption('--test_title', action='store', default=None,
                    help='single testcase base on test_title')
...
def pytest_generate_tests(metafunc):
    ...
    """
    options = ['ruledir', 'ruledir_recurse', 'rule', 'test_title']
